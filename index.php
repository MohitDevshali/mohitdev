<!DOCTYPE html>
<html>
<body>

<?php
echo "<p align='center'> <font color=blue  size='6pt'>Application is successfully deployed from Git and Github </font> </p>";
echo ' <img src="mohit.png" style="width: 20%; height: 20%" alt='"I am currently working as a Middleware administator for project Bayer in capgemini, in which I have learnt about 
middleware components and services. I also have deployement through jenkins for aws servers with the help of docker build number"'/>'; 

echo "<p align='center'> <font color=black  size='4pt'> Mohit Devshali Resume</font> </p>"
?>
<iframe src="MOHITD.pdf" style="width: 100%; height: 100vh"></iframe>


</body>
</html>
